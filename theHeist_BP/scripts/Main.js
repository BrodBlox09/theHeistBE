...
import { EntityTypes, BlockPermutation, EffectTypes, system, world, DynamicPropertiesDefinition } from "@minecraft/server";
import * as dataManager from "imports/entity_dynamic_properties.js";
import "lvl_loader.js";
import "gameband.js";
import "alarm.js";
...
world.beforeEvents.chatSend.subscribe(event => {
    ...
    switch (cmd) {
        case "lvlTp":
            ...
            break;
        case "tellRot":
            var xRot = player.getRotation().x;
            var yRot = player.getRotation().y;
        ...
        case "setAlarmLvl":
            ...
            var playerEnergyTag = player.getTags().find((x) => (JSON.parse(x).name == "energyTracker"));
            var playerEnergyTagJSON = undefined;
            var playerLevelInformationTag = player.getTags().find((x) => (JSON.parse(x).name == "levelInformation"));
            var playerLevelInformationTagJSON = undefined;
            try {
                playerEnergyTagJSON = JSON.parse(playerEnergyTag);
                playerLevelInformationTagJSON = JSON.parse(playerLevelInformationTag);
            ...
            // 9 * 100 - 158
            //var alarmLvlXpVal = (((Number(args[0]) / 100) - 0.06) * 742) + 41;
        ...
        //}*/
            ...
            var newPlayerLevelInformationTag = JSON.stringify(playerLevelInformationTagJSON);
            system.run(() => {
        ...
        case "tpCam":
            system.run(() => {
                var camera = world.getDimension("overworld").getEntities({
        ...
        case "tpFacing":
            system.run(() => {
                ...
                player.sendMessage(`X: ${cos(player.getRotation().x) * 0.7} Z: ${sin(player.getRotation().x) * 0.7}`);
            });
            break;
        case "setBlock":
            system.run(() => {
                var block = {"type": "computer", "x": -22, "y": -58, "z": 58, "rotation": 5};
                ...
                setBlock(player.location, `theheist:${block.type}`, {"theheist:unlocked": 2, "theheist:rotation": 1});
            });
            break;
    }
});
