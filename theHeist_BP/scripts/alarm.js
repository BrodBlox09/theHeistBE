...
system.runInterval(() => {
    ...
    var level = undefined;
    if (playerLevelInformationDataNode)
        level = playerLevelInformationDataNode.information[1].level;
    if (playerLevelInformationDataNode == undefined || level == undefined || level > 0)
        return;
    ...
    if ((system.currentTick % 15 == 0)) {
        ...
        cameraMappingArmorStands.forEach((armorStand) => {
            ...
            if (blockAtLevel && blockAtLevel.typeId == "minecraft:air")
                overworld.fillBlocks(belowBlock, belowBlock, BlockPermutation.resolve("theheist:camera_sight"));
            else
                armorStand.kill();
        });
    }
