...
function updateCameras(player: Player, levelCI: ILevelCloneInfo) {
	...
	const cameraMappingQuery: EntityQueryOptions = {
		"type": "armor_stand",
		"location": { 'x': player.location.x, 'y': Utilities.raycastHeight, 'z': player.location.z },
		"maxDistance": movingSecurityDeviceLoadingRange,
		"tags": ["camera"]
	};
	const cameraMappingArmorStands = Utilities.dimensions.overworld.getEntities(cameraMappingQuery).filter((x) => (x.location.y == Utilities.raycastHeight));
	...
	if (system.currentTick % 20 == 0) {
		...
		cameraArmorStands.forEach(armorStand => {
			...
			for (let i = 0; i < maxCount; i++) {
				let rayArmorStand = Utilities.dimensions.overworld.spawnEntity("minecraft:armor_stand", { "x": armorStand.location.x, "y": Utilities.raycastHeight, "z": armorStand.location.z });
				...
				if (cameraTrackerDataNode.isRobot) {
					rayArmorStand.teleport({ "x": armorStand.location.x + -(Utilities.sin(armorStand.getRotation().y) * 0.7), "y": Utilities.raycastHeight, "z": armorStand.location.z + (Utilities.cos(armorStand.getRotation().y) * 0.7) });
				}
			}
			...
			Utilities.setBlock({ "x": armorStand.location.x, "y": Utilities.cameraBlockCacheMappingHeight, "z": armorStand.location.z }, "air");
		});
...
function updateSonars(player: Player, levelCI: ILevelCloneInfo) {
	Utilities.fillBlocks(new Vector(levelCI.startX, Utilities.sonarBlockMappingHeight, levelCI.startZ), new Vector(levelCI.endX, Utilities.sonarBlockMappingHeight, levelCI.endZ), "air");
	...
	const sonarMappingArmorStands = Utilities.dimensions.overworld.getEntities(sonarMappingQuery).filter((x) => (x.location.y == Utilities.raycastHeight));
	...
	if ((system.currentTick % 15 == 0)) {
		...
		sonarArmorStands.forEach((armorStand) => {
			...
			for (var i = 0; i < maxCount; i++) {
				var rayArmorStand = Utilities.dimensions.overworld.spawnEntity("minecraft:armor_stand", { "x": armorStand.location.x, "y": Utilities.raycastHeight, "z": armorStand.location.z });
		...
		sonarMappingArmorStands.forEach((armorStand) => {
			...
			armorStand.teleport({ "x": armorStand.location.x + -(Utilities.sin(armorStand.getRotation().y) * tpDistance), "y": Utilities.raycastHeight, "z": armorStand.location.z + (Utilities.cos(armorStand.getRotation().y) * tpDistance) }, { 'dimension': Utilities.dimensions.overworld });
			let armorStandLocationAbove = Vector.from({ "x": armorStand.location.x, "y": Utilities.levelPlayingHeight, "z": armorStand.location.z });
			let armorStandLocationBelow = Vector.from({ "x": armorStand.location.x, "y": Utilities.sonarBlockMappingHeight, "z": armorStand.location.z });
...
function updateSonar360s(player: Player) {
	...
	if ((system.currentTick % 15 == 0)) {
		...
		sonarArmorStands.forEach((armorStand) => {
			...
			//system.runTimeout(() => {
				for (var i = 0; i < maxCount; i++) {
					var rayArmorStand = Utilities.dimensions.overworld.spawnEntity("minecraft:armor_stand", { "x": armorStand.location.x + 0.6, "y": Utilities.raycastHeight, "z": armorStand.location.z });
				...
				for (var i = 0; i < maxCount; i++) {
					var rayArmorStand = Utilities.dimensions.overworld.spawnEntity("minecraft:armor_stand", { "x": armorStand.location.x, "y": Utilities.raycastHeight, "z": armorStand.location.z + 0.6 });
					rayArmorStand.addTag("sonar360");
